import type { ContractsListRow } from "@/modules/contracts/domain/contracts-list-view-model";
import {
...
export function ContractsListTable({ rows }: { rows: ContractsListRow[] }) {
  if (rows.length === 0) {
    return (
              ...
              <BorderlessCell>
                <Link href={`/contracts/${contract.id}`} className="group block">
                  <div className="flex items-center gap-2.5">
                    <DicebearAvatar seed={contract.id} style="triangles" className="size-6" />
