...
import type { VerifyContractResult } from "@/modules/registry/types";
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const result = await proxyAtlasPost<VerifyContractResult>(
    `/v1/schema-projects/${id}/verify/contract`,
    body,
    token,
  );
